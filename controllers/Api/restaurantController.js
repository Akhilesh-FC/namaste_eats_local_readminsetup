...
exports.updateOrderStatus_working = async (req, res) => {
  try {
    const { restaurant_id, order_id, is_accept } = req.body;
    ...
    const order = await Order.findOne({
      where: { order_id, restaurant_id },
      attributes: ["id", "order_id", "user_id", "restaurant_id", "order_status","paymode","is_cod_paid",],
    });
    ...
    await Order.update({ order_status: newStatus }, { where: { order_id, restaurant_id } });
	   ...
	   // 🟢 STEP 1: COD auto-payment update
    if (newStatus === "DELIVERED" && order.paymode === "cod") {
      await Order.update(
        { is_cod_paid: 1 },
        { where: { order_id, restaurant_id } }
      );
      console.log(`✅ COD payment marked as paid for order_id ${order_id}`);
    }

    // 🟢 STEP: Update delivery_boy_id from Firebase if assigned or accepted
    if (["ASSIGN_TO_DELIVERY_BOY", "ORDER_ACCEPT_BY_DB"].includes(newStatus)) {
      try {
        const orderRef = firestore.collection("orders").doc(order_id.toString());
        const orderSnap = await orderRef.get();
        ...
        if (orderSnap.exists) {
          ...
          console.log(`⚠️ Order ${order_id} not found in Firestore`);
        }
      } catch (fbErr) {
        console.error("❌ Firebase delivery ID fetch error:", fbErr.message);
      }
    ...
    // 🔹 Get user details for FCM notification
    ...
    if (user && user.fcm_token) {
      ...
      switch (newStatus) {
        case "ORDER_ACCEPT":
          ...
          notificationBody = "Your order has been accepted by the restaurant.";
          break;
        case "PREPARING":
          notificationTitle = "Order Preparing 👨‍🍳";
          notificationBody = "Your food is being prepared.";
          break;
        case "READY_TO_PICKUP":
          notificationTitle = "Ready to Pickup 🚀";
          notificationBody = "Your order is ready to be picked up.";
          break;
        case "ON_THE_WAY":
          notificationTitle = "Order on the Way 🚗";
          notificationBody = "Your food is on the way to you!";
          break;
        case "DELIVERED":
      ...
      try {
        ...
        if (notifyErr.errorInfo?.code === "messaging/registration-token-not-registered") {
          console.warn(`⚠️ Invalid FCM token for user ${user.id}. Clearing token...`);
    ...
    // 🔹 Trigger internal API (READY_TO_PICKUP only)
    if (newStatus === "READY_TO_PICKUP") {
      try {
        console.log("🚀 Triggering test-push-orders API...");
        const response = await axios.post("https://root.namasteats.com/api/deliveryboy/test-push-orders", {
          order_id,
          restaurant_id,
        });
        console.log("✅ test-push-orders API Response:", response.data);
      } catch (pushErr) {
        console.error("❌ Error triggering test-push-orders API:", pushErr.message);
      }
    }
...
exports.updateRestaurantProfile = async (req, res) => {
  try {
    ...
    if (!restaurant_id) {
      return res.status(400).json({ status: 400, message: "restaurant_id is required" });
    }

    if (!mobile && !email && !req.file) {
      return res.status(400).json({ status: 400, message: "At least one field (mobile, email, or image) is required to update" });
    }
    ...
    const updatedRestaurant = await Restaurant.findOne({
      where: { id: restaurant_id },
      attributes: ["id", "name", "email", "mobile", "owner_name", "owner_mobile", "is_active", "image"],
    });

    return res.status(200).json({ status: 200, message: "Restaurant profile updated successfully", data: updatedRestaurant });
  } catch (error) {
...
exports.getRestaurantProducts = async (req, res) => {
  try {
    ...
    const restaurant = await Restaurant.findOne({
      where: { id: restaurant_id },
      attributes: ["id", "name", "distance", "address", "image", "cod_available", "veg_type", "is_active", "video", "rating", "cooking_time", "latitude", "longitude"],
      include: [
        {
          model: RestaurantOffer,
          as: "offers",
    ...
    const [products, total_products] = await Promise.all([
      ...
      Product.count({ where: { restaurant_id } }),
    ]);
...
// ✅ New API: getRestaurantProducts with limit, offset, total_products
exports.getRestaurantProductsPaginated = async (req, res) => {
  try {
    const { restaurant_id } = req.query;
    ...
    console.error("❌ Error in getRestaurantProductsPaginated:", error);
    return res.status(500).json({
      status: false,
      message: "Server error",
      error: error.message,

...
// uuid replacement - no external package needed
...
const sendOtp = async (req, res) => {
  try {
    ...
    const url = `https://indopay.cloud/otp/newsend_otp.php?merchant_key=e280cfc2e0db83480de11bb607ef7ce72f8b98f3420cb984&mobile_no=${mobile}&digit=4`;
    ...
    const response = await axios({
      ...
      httpsAgent: agent,
      headers: {
        "User-Agent": "Mozilla/5.0", // 🔥 important (some APIs block axios)
      },
    });
    ...
    console.log("ERROR FULL:", error.code, error.message);
...
const sendOtp_old = async (req, res) => {
  try {
    const { mobile } = req.body;
